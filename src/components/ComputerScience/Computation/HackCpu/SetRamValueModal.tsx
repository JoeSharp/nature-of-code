...
import ButtonBar, { Props as ButtonBarProps } from "src/components/Bootstrap/Buttons/ButtonBar";
...
const SetRamValueModal: React.FunctionComponent<Props> = ({
    ...
    return <ModalDialog {...rest} header={<h3>Set a value in RAM</h3>} content={<form>
        <div className='form-group'>
            <label>Start Address</label>
            <input className='form-control' value={startAddress} onChange={onStartAddressChange} />
        </div>
        <div className='form-group'>
            <label>Memory Value (s)</label>
            <input className='form-control' value={memoryValue} onChange={onMemoryValueChange} />
        </div>
    </form>}
...
const useSetRamValueModal = ({ onConfirm }: NewProps): UseSetRamValueModal => {
    ...
    const onStartAddressChange: React.ChangeEventHandler<HTMLInputElement> =
        React.useCallback(({ target: { value } }) => setStartAddress(parseInt(value, 10)), [setStartAddress]);
    const onMemoryValueChange: React.ChangeEventHandler<HTMLInputElement> =
        React.useCallback(({ target: { value } }) => setMemoryValue(value), [setMemoryValue]);

    const _onConfirm = React.useCallback(() => {
        onConfirm(startAddress, memoryValue.split(' ').map(s => parseInt(s, 10)))
